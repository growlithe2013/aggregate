...
func (q *Queries) GetFeedFollows(ctx context.Context, userID uuid.UUID) ([]uuid.UUID, error) {
...
const getFeedIDsByUserID = `-- name: GetFeedIDsByUserID :many
SELECT feed_id FROM feed_follows
WHERE user_id = $1
`

func (q *Queries) GetFeedIDsByUserID(ctx context.Context, userID uuid.UUID) ([]uuid.UUID, error) {
	rows, err := q.db.QueryContext(ctx, getFeedIDsByUserID, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
